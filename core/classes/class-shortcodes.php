...
class Odin_Shortcodes {
	...
	function button( $atts, $content = null ) {
		...
		$html .= ( $tooltip ) ? ' race-chrome-tooltip' : '';
		$html .= '"';
	...
	function tabs( $atts, $content = null ) {
		return '<ul class="nav nav-tabs race-chrome-tabs">' . str_replace( '<br />', '', do_shortcode( $content ) ) . '</ul>';
	}
	...
	function accordions( $atts, $content = null ) {
		extract( shortcode_atts( array(
			'id' => 'race-chrome-accordion',
		), $atts ) );

		$html = '<div class="panel-group race-chrome-accordion" id="' . esc_attr( $id ) . '">';
		$html .= str_replace( '<br />', '', do_shortcode( $content ) );
		$html .= '</div>';
	...
	function tooltip( $atts, $content = null ) {
		...
		$html = '<a class="race-chrome-tooltip" data-original-title="' . esc_attr( $title ) . '" href="' . esc_url( $link ) .'" data-placement="' . esc_attr( $direction ) . '" data-toggle="tooltip">';
		$html .= do_shortcode( $content );
		$html .= '</a>';
	...
	function map( $atts, $content = null ) {
		...
		$html = '<div class="race-chrome-map" id="' . esc_attr( $id ) . '" style="width: ' . esc_attr( $width ) . 'px; height: ' . esc_attr( $height ) . 'px;"></div>';

		$js = '<script type="text/javascript" src="//maps.google.com/maps/api/js?sensor=false"></script>';
		$html .= apply_filters( 'race_chrome_map_shortcode_js_' . $id, $js );
