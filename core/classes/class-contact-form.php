...
class Odin_Contact_Form extends Odin_Front_End_Form {
	...
	public function __construct( $id, $to, $cc = array(), $bcc = array(), $attributes = array(), $attachment_type = 'file' ) {
		...
		// Hooks send_mail.
		add_action( 'race_chrome_front_end_form_submitted_data_' . $this->id, array( $this, 'send_mail' ), 1, 2 );
	}
	...
	protected function build_mail_message( $submitted_data, $attachments ) {
		...
		$message = apply_filters( 'race_chrome_contact_form_message_header_' . $this->id, '' );
		...
		$message .= apply_filters( 'race_chrome_contact_form_message_footer_' . $this->id, '' );
	...
	protected function build_mail_subject( $submitted_data ) {
		if ( ! empty( $this->subject ) ) {
			...
			// Default subject.
			return sprintf(
				__( 'Message sent by the form %s in %s at %s', 'race-chrome' ),
				$this->id,
	...
	protected function format_mail_headers( $submitted_data ) {
		...
		return apply_filters( 'race_chrome_contact_form_mail_headers_' . $this->id, $headers );
	}
