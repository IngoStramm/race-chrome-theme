...
class Odin_Metabox {
	...
	public function scripts() {
		...
		if ( in_array( $screen->id, $this->get_post_type() ) ) {
			...
			// Metabox.
			wp_enqueue_script( 'race-chrome-admin', get_template_directory_uri() . '/core/assets/js/admin.js', array( 'jquery' ), null, true );
			wp_enqueue_style( 'race-chrome-admin', get_template_directory_uri() . '/core/assets/css/admin.css', array(), null, 'all' );
			...
			wp_localize_script(
				...
				'odinAdminParams',
				array(
					'galleryTitle'  => __( 'Add images in gallery', 'race-chrome' ),
					'galleryButton' => __( 'Add in gallery', 'race-chrome' ),
					'galleryRemove' => __( 'Remove image', 'race-chrome' ),
					'uploadTitle'   => __( 'Choose a file', 'race-chrome' ),
					'uploadButton'  => __( 'Add file', 'race-chrome' ),
				)
			);
		}
	}
	...
	public function metabox( $post ) {
		...
		do_action( 'race_chrome_metabox_header_' . $this->id, $post_id );

		echo apply_filters( 'race_chrome_metabox_container_before_' . $this->id, '<table class="form-table race-chrome-form-table">' );

		foreach ( $this->fields as $field ) {
			echo apply_filters( 'race_chrome_metabox_wrap_before_' . $this->id, '<tr valign="top">', $field );

			if ( 'title' == $field['type'] ) {
				$title = sprintf( '<th colspan="2"><strong>%s</strong></th>', $field['label'] );
			} elseif ( 'separator' == $field['type'] ) {
				$title = sprintf( '<td colspan="2"><span id="race-chrome-metabox-separator-%s" class="race-chrome-metabox-separator"></span></td>', $field['id'] );
			} else {
				$title = sprintf( '<th><label for="%s">%s</label></th>', $field['id'], $field['label'] );
			}

			echo apply_filters( 'race_chrome_metabox_field_title_' . $this->id, $title, $field );

			echo apply_filters( 'race_chrome_metabox_field_before_' . $field['id'], '<td>', $field );
			...
			echo apply_filters( 'race_chrome_metabox_field_after_' . $field['id'], '</td>', $field );

			echo apply_filters( 'race_chrome_metabox_wrap_after_' . $this->id, '</tr>', $field );
		}

		echo apply_filters( 'race_chrome_metabox_container_after_' . $this->id, '</table>' );

		do_action( 'race_chrome_metabox_footer_' . $this->id, $post_id );
	...
	protected function process_fields( $args, $post_id ) {
		...
		switch ( $type ) {
			...
			case 'editor':
				...
				break;
			case 'color':
				$this->field_input( $id, $current, array_merge( array( 'class' => 'race-chrome-color-field' ), $attrs ) );
				break;
			case 'upload':
			...
			default:
				do_action( 'race_chrome_metabox_field_' . $this->id, $type, $id, $current, $options, $attrs );
				break;
		}
	}
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button race-chrome-upload-button" type="button" value="%3$s" />', $id, esc_url( $current ), __( 'Select file', 'race-chrome' ), $this->build_field_attributes( $attrs ) );
	}
	...
	protected function field_image( $id, $current ) {
		...
		$image = apply_filters( 'race_chrome_placeholder_path', get_template_directory_uri() . '/core/assets/images/placeholder.png' );
		$html  = '<div class="race-chrome-upload-image">';
	...
	protected function field_image_plupload( $id, $current ) {
		$html = '<div class="race-chrome-gallery-container">';
			$html .= '<ul class="race-chrome-gallery-images">';
				if ( ! empty( $current ) ) {
			...
			$html .= sprintf( '<input type="hidden" class="race-chrome-gallery-field" name="%s" value="%s" />', $id, $current );

			// Adds "adds images in gallery" url.
			$html .= sprintf( '<p class="race-chrome-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Add images in gallery', 'race-chrome' ) );
		$html .= '</div>';
	...
	public function save( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if ( ! in_array( $field['type'], array( 'separator', 'title' ) ) ) {
				...
				$new = apply_filters( 'race_chrome_save_metabox_' . $this->id, $value, $name );
