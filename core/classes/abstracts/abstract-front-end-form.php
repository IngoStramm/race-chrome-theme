...
abstract class Odin_Front_End_Form {
	...
	protected function process_fields() {
		...
		if ( ! empty( $this->fields ) ) {
			foreach ( $this->fields as $key => $fieldset ) {
				...
				$html .= sprintf( '<fieldset id="race-chrome-form-fieldset-%s" %s>', $key, $this->process_attributes( $fieldset_attributes ) );
				...
				foreach ( $fieldset['fields'] as $field ) {
					...
					switch ( $type ) {
						...
						default:
							$html .= do_action( 'race_chrome_front_end_form_field_' . $this->id, $id, $label, $default, $description, $attributes, $options );
							break;
					}
				}
	...
	protected function process_buttons() {
		...
		if ( ! empty( $this->buttons ) ) {
			...
			$html .= '<button type="submit" class="btn btn-primary">' . __( 'Submit', 'race-chrome' ) . '</button>';
		}
	...
	protected function display_success_message() {
		...
		if ( isset( $_GET['success'] ) && 1 == $_GET['success'] ) {
			...
			if ( ! empty( $this->success ) ) {
				...
				$html .= '<p>' . __( 'Form submitted successfully!', 'race-chrome' ) . '</p>';
			}
			$html .= '</div>';
		}
	...
	protected function field_input( $id, $label, $default, $description, $attributes ) {
		...
		$html = sprintf( '<div class="form-group race-chrome-form-group-%s">', $id );
	...
	protected function field_checkbox( $id, $label, $default, $description, $attributes ) {
		...
		$html = sprintf( '<div class="checkbox race-chrome-form-group-%s">', $id );
			...
			foreach ( $this->fields as $fieldset ) {
				foreach ( $fieldset['fields'] as $field ) {
					...
					if ( $type != 'file' && $required && empty( $data[ $id ] ) ) {
						$this->set_errors( sprintf( __( '%s is required.', 'race-chrome' ), '<strong>' . $label . '</strong>' ) );
					}

					switch ( $type ) {
						case 'email':
							if ( ! is_email( $value ) ) {
								$this->set_errors( sprintf( __( '%s must be an email address valid.', 'race-chrome' ), '<strong>' . $label . '</strong>' ) );
							}
							break;
						case 'file':
							if ( count($files) >= 1 ) {
						...
						default:
							$custom_message = apply_filters( 'race_chrome_front_end_form_valid_' . $this->id . '_' . $id, '', $label, $value );
							if ( $custom_message ) {
	...
	protected function redirect() {
		...
		$url = apply_filters( 'race_chrome_front_end_form_redirect_' . $this->id, add_query_arg( 'success', '1', $url ) );
			...
			foreach ( $fieldset['fields'] as $field ) {
				...
				if ( 'file' == $field['type'] && isset( $_FILES[ $id ] ) ) {
					...
					if ( is_wp_error( $attachment_id ) && $_FILES[ $id ]['size'] > 0 ) {
						$error = apply_filters( 'race_chrome_front_end_form_upload_error_' . $this->id, sprintf( '%s %s.', '<strong>' . $this->get_field_label( $id ) . '</strong>', $attachment_id->get_error_message() ) );
	...
	public function init() {
		...
		if ( ! empty( $submitted_data ) && isset( $submitted_data['race_chrome_form_action'] ) && $this->id == $submitted_data['race_chrome_form_action'] ) {
			// Validates the form data.
			$this->validate_form_data();

			if ( $this->is_valid() ) {
				// Hook to process submitted form data.
				do_action( 'race_chrome_front_end_form_submitted_data_' . $this->id, $submitted_data, $uploaded_files );

				// Redirect after submit.
				$this->redirect();
			} else {
				add_filter( 'race_chrome_front_end_form_messages_' . $this->id, array( $this, 'display_error_messages' ) );
			}
		}
	}
	...
	public function render() {
		...
		// Display error messages.
		$html .= apply_filters( 'race_chrome_front_end_form_messages_' . $this->id, $html );
		...
		$html .= sprintf(
			...
			$html .= do_action( 'race_chrome_front_end_form_before_fields_' . $this->id );
			$html .= $fields;
			$html .= do_action( 'race_chrome_front_end_form_after_fields_' . $this->id );
			$html .= $this->process_buttons();
			$html .= sprintf( '<input type="hidden" name="race_chrome_form_action" value="%s" />', $this->id );
		$html .= '</form>';
