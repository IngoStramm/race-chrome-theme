 ...
 */
function race_chrome_pagination( $mid = 2, $end = 1, $show = false, $query = null ) {
	...
	if ( ! isset( $_GET[ 'infinity' ] ) ) {
		...
		if ( $total_pages > 1 ) {
			...
			$arguments = apply_filters( 'race_chrome_pagination_args', array(
					...
					'mid_size'  => $mid,
					'type'      => 'list',
					'prev_text' => __( '&laquo; Previous', 'race-chrome' ),
					'next_text' => __( 'Next &raquo;', 'race-chrome' ),
				)
			);
 ...
 * Add in single.php <?php race_chrome_related_posts(); ?>
 * To show related by tags:
 * Add in single.php <?php race_chrome_related_posts( 'tag' ); ?>
 *
 * @since  2.2.0
 *
 ...
 */
function race_chrome_related_posts( $display = 'category', $qty = 4, $title = '', $thumb = true, $post_type = 'post' ) {
	global $post;

	$show = false;
	$post_qty = (int) $qty;
	! empty( $title ) || $title = __( 'Related Posts', 'race-chrome' );
			...
			while ( $related->have_posts() ) {
				...
				if ( $thumb ) {
					if ( has_post_thumbnail() ) {
						...
						$img = '<img src="' . get_template_directory_uri() . '/core/assets/images/race-chrome-thumb-placeholder.jpg" alt="' . get_the_title() . '">';
					}
					// Filter to replace the image.
					$image = apply_filters( 'race_chrome_related_posts_thumbnail', $img );
 ...
 *
 * Usage:
 * Place: <?php echo race_chrome_excerpt( 'excerpt', value ); ?>
 *
 * @since  2.2.0
 *
 ...
 */
function race_chrome_excerpt( $type = 'excerpt', $limit = 40 ) {
	$limit = (int) $limit;
 ...
 */
function race_chrome_breadcrumbs( $homepage = '' ) {
	global $wp_query, $post, $author;

	! empty( $homepage ) || $homepage = __( 'Home', 'race-chrome' );
	...
	if ( ! is_home() && ! is_front_page() || is_paged() ) {
		...
		// Category archive.
			...
			printf( __( '%sCategory: %s%s', 'race-chrome' ), $current_before, single_cat_title( '', false ), $current_after );

		// Tags archive.
		} elseif ( is_tag() ) {
			printf( __( '%sTag: %s%s', 'race-chrome' ), $current_before, single_tag_title( '', false ), $current_after );
		...
		// Search page.
			...
			printf( __( '%sSearch result for: &quot;%s&quot;%s', 'race-chrome' ), $current_before, get_search_query(), $current_after );

		// Author archive.
		} elseif ( is_author() ) {
			$userdata = get_userdata( $author );

			echo $current_before . __( 'Posted by', 'race-chrome' ) . ' ' . $userdata->display_name . $current_after;
		...
		// 404 page.
			...
			echo $current_before . __( '404 Error', 'race-chrome' ) . $current_after;
		}
 ...
 * @return string
 */
function race_chrome_get_image_url( $id, $width, $height, $crop = true, $upscale = false ) {
 ...
 */
function race_chrome_thumbnail( $width, $height, $alt = '', $crop = true, $class = '', $upscale = false ) {
	...
	if ( $thumb ) {
		$image = race_chrome_get_image_url( $thumb, $width, $height, $crop, $upscale );
		...
		return apply_filters( 'race_chrome_thumbnail_html', $html );
	}
}
 ...
 *
 * Use:
 * add_action( 'the_post', 'race_chrome_autoset_featured' );
 * add_action( 'save_post', 'race_chrome_autoset_featured' );
 * add_action( 'draft_to_publish', 'race_chrome_autoset_featured' );
 * add_action( 'new_to_publish', 'race_chrome_autoset_featured' );
 * add_action( 'pending_to_publish', 'race_chrome_autoset_featured' );
 * add_action( 'future_to_publish', 'race_chrome_autoset_featured' );
 *
 * @since  2.2.0
 *
 * @global array $post WP post object.
 */
function race_chrome_autoset_featured() {
	global $post;
 ...
 */
function race_chrome_debug( $variable ) {
 ...
 *
 * Usage:
 * <?php echo race_chrome_get_term_meta( $term_id, $field );?>
 *
 * @since  2.2.7
 *
 ...
 */
function race_chrome_get_term_meta( $term_id, $field ) {
	...
	$option_name = sprintf( 'race_chrome_term_meta_%s_%s', $term_id, $field );
