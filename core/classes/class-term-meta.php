...
class Odin_Term_Meta {
	...
	public function edit_view() {
		...
		foreach ( $this->fields as $field ) {
			...
			echo apply_filters( 'race_chrome_term_meta_field_edit_screen_before_' . $this->id, '<td>', $field );
			$this->process_fields( $field );
			...
			echo apply_filters( 'race_chrome_term_meta_field_edit_screen__after_' . $this->id, '</td>', $field );
	...
	public function delete_fields( $term ) {
		...
		$option = sprintf( 'race_chrome_term_meta_%s', $term );
	...
	protected function get_value( $id, $field ) {
		...
		$option_name = sprintf( 'race_chrome_term_meta_%s_%s', $id, $field );
	...
	protected function process_fields( $args ) {
		...
		switch ( $type ) {
			...
			default :
				do_action( 'race_chrome_user_meta_field_' . $this->id, $type, $id, $current, $options, $attrs );
				break;
		}
	}
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button race-chrome-upload-button" type="button" value="%3$s" /><br />', $id, esc_url( $current ), __( 'Select file', 'race-chrome' ), $this->build_field_attributes( $attrs ) );
	}
	...
	public function save( $term_id, $tt_id = null ) {
		...
		foreach ( $this->fields as $field ) {
			...
			$new  = apply_filters( 'race_chrome_save_term_meta_' . $this->id, $_POST[ $name ], $name );
