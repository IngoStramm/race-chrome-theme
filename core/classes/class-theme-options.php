...
class Odin_Theme_Options {
	...
	public function callback_color( $args ) {
		// Sets color class.
		$args['attributes']['class'] = 'race-chrome-color-field';
	...
	public function callback_upload( $args ) {
		...
		$html = sprintf( '<input type="text" id="%1$s" name="%2$s[%1$s]" value="%3$s" class="regular-text"%5$s /> <input class="button race-chrome-upload-button" id="%1$s-button" type="button" value="%4$s" />', $id, $tab, $current, __( 'Select file', 'race-chrome' ), $this->build_field_attributes( $attrs ) );
	...
	public function callback_image_plupload( $args ) {
		...
		$html = '<div class="race-chrome-gallery-container">';
			...
			$html .= sprintf( '<input type="hidden" id="%1$s" name="%2$s[%1$s]" value="%3$s" class="race-chrome-gallery-field" />', $id, $tab, $current );
	...
	public function validate_input( $input ) {
		...
		foreach ( $input as $key => $value ) {
			...
			if ( isset( $input[ $key ] ) ) {
				$output[ $key ] = apply_filters( 'race_chrome_theme_options_validate_' . $this->id, $value, $key );
			}
