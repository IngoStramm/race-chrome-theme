...
class Odin_Post_Status {
	...
	public function __construct( $post_status, $post_types, $args ) {
		...
		if( ! isset( $this->args['label_count'] ) ) {
			$this->args['label_count'] = _n_noop( $this->applied_label . '&nbsp;<span class="count">(%s)</span>', $this->applied_label . '&nbsp;<span class="count">(%s)</span>', 'race-chrome' );
		}
	...
	public function meta_tags() {
		...
		printf( '<meta class="race-chrome-custom-status-meta" value="%s" />', esc_attr( json_encode( $args ) ) );
	}
	...
	public function scripts() {
		// Load admin JS
		wp_enqueue_script( 'race-chrome-custom-status', get_template_directory_uri() . '/core/assets/js/admin-custom-status.js', array( 'jquery' ), null, true );
