...
class Odin_Post_Form extends Odin_Front_End_Form {
	...
	public function __construct( $id, $post_type = 'post', $post_status = 'draft', $attributes = array() ) {
		...
		// Hooks save_post.
		add_action( 'race_chrome_front_end_form_submitted_data_' . $this->id, array( $this, 'save_post' ) );
	}
	...
	protected function save_custom_fields( $post_id, $submitted_data ) {
		if ( ! empty( $this->custom_fields ) ) {
			foreach ( $this->custom_fields as $key ) {
				...
				$data = apply_filters( 'race_chrome_post_form_custom_field_data_' . $this->id, $submitted_data[ $key ] );
	...
	protected function save_terms( $post_id, $submitted_data ) {
		if ( ! empty( $this->terms ) ) {
			foreach ( $this->terms as $taxonomy => $term ) {
				// Apply filter to sanitization.
				$term = apply_filters( 'race_chrome_post_form_term_data_' . $this->id, $submitted_data[ $term ] );
	...
	public function save_post( $submitted_data ) {
		if ( ! empty( $submitted_data ) ) {
			$post_data = apply_filters( 'race_chrome_post_form_insert_data_' . $this->id, array(
			...
			do_action( 'race_chrome_post_form_after_save_' . $this->id, $post_id, $submitted_data );
		}
	}
}
